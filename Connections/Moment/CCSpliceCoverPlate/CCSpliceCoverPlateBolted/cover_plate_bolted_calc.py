...
def flange_force(column_d, column_f_t,area_f,total_area, axial_force, moment_load):
    """
    Args:
    ...
    """
    tm = moment_load
    axial_force_f = area_f * axial_force / total_area
    return round((tm*1000)/(column_d - column_f_t), 2) + (axial_force_f)        # kN

# Force in flange (kN)  [Reference:  N. Subramanian (page 428)]
def web_force(column_d,column_f_t, column_t_w, axial_force, total_area):
    """
    Args:
       column_d: Overall depth of the column section in mm (float)
       column_f_t: Thickness of flange in mm (float)
    ...
    Returns:
        Force in flange in kN (float)
    """
    axial_force_w = ((column_d - 2(column_f_t)) * column_t_w * axial_force) / total_area
    return round(axial_force_w)
########################################################################################################################
########################################################################################################################
# Thickness of flange splice plate [Reference: N. Subramanian (Page 428), M.L. Gambhir (Page 10.84)]
def thk_flange_plate(column_d, column_f_t, n, gauge_req, bolt_dia, axial_force, moment_load,column_b,column_fy, dia_hole):
    """
    Args:
        column_d: Overall depth of the column section in mm (float)
        column_f_t: Thickness of flange in mm (float)
    ...
    ff = flange_force(column_d, column_f_t, axial_force, moment_load)
...
def flange_plate_w_min( flangeplatewidth):
    flangeplatewidthmin = flangeplatewidth
...
# Thickness of flange splice plate [Reference: N. Subramanian (Page 428), M.L. Gambhir (Page 10.84)]
def func_flangeinnerplatet(column_d, column_f_t, n, gauge_req, bolt_dia, axial_force, moment_load,column_b,column_fy, dia_hole):
    """
    Args:
        column_d: Overall depth of the column section in mm (float)
        column_f_t: Thickness of flange in mm (float)
...
def web_block_shear_axial( end_dist, number_of_row_web, column_t_w, dia_hole, column_d, column_f_t, number_of_column_web, fy, fu):
    """
    Args:
...
def coverplateboltedconnection(uiObj):
    ...
    if web_plate_t < float(column_t_w / 2):  ## Web plate thickness = web plate 1 thickness = (thickness of web / 2)
        ...
        logger.warning(": Maximum allowed thickness of web splice plate is %2.2f mm" % (web_max_t(bolt_diameter)))
...
## Note: As per discussion with Prof. Siddhartha Ghosh below flange_gauge calculation is implemented on 27-July-2018
    ...
    if flange_plate_preference == "Outside":
        ...
        flangeminw = flange_plate_w_min(edge_dist, number_of_column_flange)
        ...
        flange_pitch = flange_gauge
        flangeminw = flange_plate_w_min(edge_dist, number_of_column_flange, flange_gauge)
        if flange_plate_w < flangeminw and flange_plate_w > column_b:
            design_status = False
            logger.error(": Flange width available is not sufficient")
            logger.warning(": width of flange should be between flangeminw and column_b")
    ...
    thkflangeplate = thk_flange_plate(column_d, column_f_t, axial_force, moment_load, column_b, column_fy, dia_hole)
    ...
    A_vn = (web_plate_w - n_bolts * 2 * dia_hole) * web_plate_t
    R_n = shear_rupture(A_vn, column_fu)
    if R_n < shear_load:
        design_status = False
