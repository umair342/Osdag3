...
class EndPlate(object):
	def __init__(self, input_dict, output_dict, beam_data, folder):
		...
		self.flange_weld_thickness = float(input_dict['Weld']['Flange (mm)'])  # 12
		self.web_weld_thickness = float(input_dict["Weld"]['Web (mm)'])  # 8
	...
	def save_to_svg(self, filename, view):
		...
		if view == "Front":
			...
			extnd_bothway_end_2d_front.call_ExtndBoth_front(filename)
			cairosvg.svg2png(file_obj=open(filename,'rb'), write_to=os.path.join(str(self.folder), "images_html", "extendFront.png"))
			...
			extnd_bothway_end_2d_side.call_ExtndBoth_side(filename)
			cairosvg.svg2png(file_obj=open(filename,'rb'), write_to=os.path.join(str(self.folder), "images_html", "extendSide.png"))
...
class ExtendedEnd2DFront(object):
	...
	def call_ExtndBoth_front(self, filename):
		...
		if self.data_object.input_dict != "Flush":
			...
			theta = 60
			offset = 50
			textup = str(no_of_bolts_flange) + " nos " + str(self.data_object.bolt_hole_diameter) + '\u00d8' + " holes"
...
class ExtendedEnd2DSide(object):
	...
	def __init__(self, extnd_common_object):
		...
		# =========================  End Plate 1  =========================
		ptP1x = 0.0
		ptP1y = 0.0  # (self.data_object.plate_length_L1 + self.data_object.beam_depth_D1)/2
		self.P1 = np.array([ptP1x, ptP1y])
