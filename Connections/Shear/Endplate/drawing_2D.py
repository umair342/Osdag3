...
class EndCommonData(object):
    def __init__(self, input_obj, ouput_obj, dict_beam_data, dict_column_data, folder):
        ...
        self.D_beam = float(dict_beam_data["D"])
        self.D_col = float(dict_column_data["D"])
        self.col_B = float(dict_column_data["B"])
        self.beam_B = float(dict_beam_data["B"])
    ...
    def save_to_svg(self, filename, view):
        ...
        if self.connectivity == 'Column flange-Beam web':
            if view == "Front":
                ...
                cairosvg.svg2png(file_obj=open(filename,'rb'), write_to= os.path.join(str(self.folder), "images_html", "endFront.png"))

                filename = os.path.join(str(self.folder), "images_html", "endSide.svg")
                end_2d_side.call_CFBW_side(filename)
                cairosvg.svg2png(file_obj=open(filename,'rb'), write_to=os.path.join(str(self.folder), "images_html", "endSide.png"))

                filename = os.path.join(str(self.folder), "images_html", "endTop.svg")
                end_2d_top.call_CFBW_top(filename)
                cairosvg.svg2png(file_obj=open(filename,'rb'), write_to=os.path.join(str(self.folder), "images_html", "endTop.png"))
...
class End2DCreatorFront(object):
    def __init__(self, fin_common_obj):
        ...
        self.B = (self.dataObj.col_B, 0.0)
        self.A = (0.0, 0.0)
        self.D = (0.0, self.dataObj.col_L)
...
class End2DCreatorTop(object):
    def __init__(self, fin_common_obj):
        ...
        self.B = np.array([0.0, 0.0]) + (self.dataObj.col_B) * np.array([1, 0])
