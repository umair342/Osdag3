...
class FinCommonData(object):
    ...
    def __init__(self, inputObj, ouputObj, dictBeamdata, dictColumndata, folder):
        ...
        self.D_beam = float(dictBeamdata["D"])
        self.D_col = float(dictColumndata["D"])
        self.col_B = float(dictColumndata["B"])
    ...
    def saveToSvg(self, fileName, view):
        ...
        if self.connectivity == 'Column flange-Beam web':
            if view == "Front":
                ...
                fin2DFront.callCFBWfront(fileName)
                cairosvg.svg2png(file_obj=open(fileName,'rb'), write_to=os.path.join(str(self.folder), "images_html",  "finFront.png"))

                fileName = os.path.join(str(self.folder), "images_html",  "finSide.svg")
                fin2DSide.callCFBWSide(fileName)
                cairosvg.svg2png(file_obj=open(fileName,'rb'), write_to=os.path.join(str(self.folder), "images_html",  "finSide.png"))

                fileName = os.path.join(str(self.folder), "images_html",  "finTop.svg")
                fin2DTop.callCFBWTop(fileName)
                cairosvg.svg2png(file_obj=open(fileName,'rb'), write_to=os.path.join(str(self.folder), "images_html",  "finTop.png"))
