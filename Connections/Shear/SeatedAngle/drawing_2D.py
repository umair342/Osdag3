...
class SeatCommonData(object):
    ...
    def __init__(self, input_dict, output_dict, beam_data, column_data, angle_data, dicttopangledata,folder):
        ...
        self.beam_depth = float(beam_data["D"])
        self.col_depth = float(column_data["D"])
        self.col_width = float(column_data["B"])
        self.beam_width = float(beam_data["B"])
    ...
    def save_to_svg(self, file_name, view):
        ...
        if self.connectivity == 'Column flange-Beam flange':
            if view == "Front":
                ...
                seat_2d_front.call_CFBF_front(file_name)
                cairosvg.svg2png(file_obj=open(file_name,'rb'), write_to=os.path.join(str(self.folder), "images_html", "seatFront.png"))

                file_name = os.path.join(str(self.folder), "images_html", "seatSide.svg")
                seat_2d_side.call_CFBF_side(file_name)
                cairosvg.svg2png(file_obj=open(file_name,'rb'), write_to=os.path.join(str(self.folder), "images_html", "seatSide.png"))

                file_name = os.path.join(str(self.folder), "images_html", "seatTop.svg")
                seat_2d_top.call_CFBF_top(file_name)
                cairosvg.svg2png(file_obj=open(file_name,'rb'), write_to=os.path.join(str(self.folder), "images_html", "seatTop.png"))
